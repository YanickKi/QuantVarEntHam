...
function RelativeEntropyBuffer(::AbstractModel{S,N_A}) where {S,N_A}
    d = Int((2*S+1)^N_A)
    return RelativeEntropyBuffer{S,N_A}(
        zeros(ComplexF64, d, d), zeros(ComplexF64, d, d), ExpBuffer(ComplexF64, d)
    )
end
...
Contains the `model`, `ansatz` and `buffer`s for the relative entropy as cost function.
...
```
"""
mutable struct RelativeEntropy{M<:AbstractModel,A<:AbstractAnsatz} <:
...
function cost_from_gradient_intermediates(fc::FixedCost{C}) where {C<:RelativeEntropy}
    unwrap(cost_from_gradient_intermediates, fc)
end

function fg!(
    F, G::Union{Vector{<:Real},Nothing}, c::Union{C,FixedCost{C}}, g::Vector{<:Real}
) where {C<:RelativeEntropy}
    if !isnothing(G)
        gradient!(G, c, g)
        if !isnothing(F)
