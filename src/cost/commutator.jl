...
function CommutatorBuffer(::AbstractModel{S,N_A}) where {S,N_A}
    d = Int((2*S+1)^N_A)
    return CommutatorBuffer{S,N_A}(
        zeros(ComplexF64, d, d), zeros(ComplexF64, d, d), zeros(ComplexF64, d, d)
    )
end
...
``C_{ij} = A_{ij} B_{ij}``.
"""
struct Commutator{M<:AbstractModel,B<:CommutatorBuffer,A<:AbstractAnsatz} <:
...
function Commutator(
    ...
    buffer::Union{Nothing,CommutatorBuffer{S,N_A}}=nothing,
) where {S,N_A}
    buffer = @something buffer CommutatorBuffer(model)

    blocks_mat = Matrix.(mat.(ansatz.blocks))

    return Commutator(model, blocks_mat, ansatz, buffer)
end
...
function gradient_component(
    c::Commutator, Λ_comm::Real, Λ_H_A::Real, Λ_ρ_A::Real, index::Integer
)
    mul!(
        mul!(c.buff.temp, c.model.ρ_A, c.blocks_mat[index]),
        c.blocks_mat[index],
        c.model.ρ_A,
        1,
        -1,
    ) # compute [h_index, ρ_A] and save it in temp
