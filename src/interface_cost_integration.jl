function make_integrator(blocks::Vector{<:AbstractMatrix}, ts::TanhSinh)
    ...
    scalar_integrator = TanhSinhScalar(q, ts.atol, ts.rtol)
    vector_integrator = TanhSinhVector(q, ts.atol, ts.rtol, buffer)

    return Integrator(scalar_integrator, vector_integrator)
end
