...
function print_block(io::IO, block::Block)
    ...
    if abs(block.prefactors[1]) == 1
        nothing
    else
        print(io, float_to_int(abs(block.prefactors[1])), "*")
    end
    ...
    for index in 2:lastindex(block.pauli_strings)
        ...
        if abs(block.prefactors[index]) == 1
            nothing
        else
            print(io, float_to_int(abs(block.prefactors[index])), "*")
        end
...
function Base.show(
    io::IO, ::MIME"text/plain", pauli_strings::Vector{PauliString{S,N,L}}
) where {S,N,L}
...
function Base.show(
    io::IO, ::MIME"text/plain", pauli_strings::Vector{PauliString{S,N}}
) where {S,N}
    println(io, "Number of Pauli strings: ", length(pauli_strings))
    println(io)
    print_info(io, pauli_strings[1])
