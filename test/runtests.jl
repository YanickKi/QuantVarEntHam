...
@testset "Phase transition at Γ=1 for TFIM" begin
    ...
    @test c_opt ≈ 0.0 atol = eps(Float64)
end
...
@testset "Classical state at Γ=0 for TFIM" begin
    ...
    for _ in 1:10
        g = rand(0.0:1e-16:5.0, 4)
        @test cost(g) ≈ 0.0 atol = eps(Float64)
    end
end
...
@testset "Fixing parameter at Γ=1 for TFIM" begin
    ...
    g_opt_free, c_opt_free = optimize(
        free_cost, g_init_free, print_result=false, show_trace=false
    )
    _g_opt_fixed, c_opt_fixed = optimize(
        fixed_cost, g_init_fixed, print_result=false, show_trace=false
    )
...
@testset "Corrections push down cost function" begin
    ...
    g_init = [[1, 2, 3, 4], [1, 1, 2, 2, 2, 3, 3], [1, 2, 3, 4], [1, 1, 2, 2, 3, 3]]
    ...
    for run in eachindex(g_init)
        _, c_opt = optimize(
            costs[run], g_init[run], ∇_tol=1e-10, print_result=false, show_trace=false
        )
        _, c_opt_corrections = optimize(
            costs_corrections[run],
            g_init_corrections[run],
            ∇_tol=1e-10,
            print_result=false,
            show_trace=false,
        )
